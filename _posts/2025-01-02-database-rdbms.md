...
### MySQL 엔진
MySQL은 다양한 스토리지 엔진을 통해 데이터를 저장하고 관리한다.
여기서 스토리지 엔진이란 데이터베이스 관리 시스템이 데이터베이스에 대해 데이터를 관리하는 데 사용하는 소프트웨어 컴포넌트이다.
스토리지 엔진에 따라 테이블 데이터와 인덱스를 관리하는 방식을 결정하지만 테이블에 대한 정의는 서버에서 관리한다.
...
클러스터형 테이블은 데이터를 기본 키 순서로 저장하며, 기본 키를 기준으로 데이터를 검색한다.
그 결과 데이터를 매우 빠르게 검색할 수 있게 된다.
