...
어떤 상황에 어떤 자료형을 사용하는 것이 성능적인 측면에서 톺아본다.

## StringBuffer / StringBuilder 클래스

`StringBuffer`와 `StringBuilder` 클래스는 문자열을 연결이나 수정하는 연산을 할 때 주로 사용하는 자료형이다.  
`String`에서는 `+` 연산이나 `concat()` 메소드를 사용할 수 있지만 `String`의 특성상 인스턴스의 문자열 값이 바뀌게 되면 새로운 String 인스턴스를 생성하게 되어,
이러한 연산을 할수록 메모리랑 성능적인 측면에서 저하가 발생된다.
...
자바에서는 `String`은 불변(Immutable) 자료형이다. 그래서 초기 값과 다른 값에 대한 연산에 많은 추가 자원을 사용하게 된다는 특징이 있다.
...
![String StringBuffer StringBuilder 속도 비교](image-3.png)
