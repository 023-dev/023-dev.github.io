...
C, C++, Go, Rust 같은 언어는 일반적으로 빌드 시점에 특정 운영체제와 CPU 아키텍처에 맞는 실행 파일을 만든다.
...
HotSpot JVM은 오래 실행되는 서버 애플리케이션에서 시작 속도와 최고 성능을 함께 얻기 위해 **단계별 컴파일(Tiered Compilation)** 을 사용한다.
Oracle의 HotSpot 성능 문서에 따르면 단계별 컴파일은 인터프리터뿐 아니라 클라이언트 컴파일러를 함께 사용해 프로파일링 정보를 모으고, 이후 서버 컴파일러가 더 강한 최적화를 적용할 수 있게 한다.
