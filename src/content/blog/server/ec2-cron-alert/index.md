...
이제 어떤 알림을 받을지 결정했고, 그럼 어떻게 알림 방식을 구성할지 생각해봐야 한다.
...
위의 스크립트도 앞서 작성한 스크립트와 유사하지만, 도커 환경에 특화된 정보들을 수집합니다.

- `CONTAINERS=$(docker ps --format "{{.Names}}")`: 실행 중인 컨테이너 목록을 가져온다.
- `STATUS=$(docker inspect --format '{{.State.Status}}' "$CONTAINER")`: 컨테이너 상태를 확인한다.
- `STATS=$(docker stats --no-stream --format "{{.CPUPerc}} {{.MemPerc}}" "$CONTAINER")`: 컨테이너의 실시간 리소스 사용량을 가져온다.
- `CPU_USAGE`, `MEMORY_USAGE`: 추출된 정보에서 퍼센트 수치만 정제하여 임계치와 비교한다.
...
> 위와 같이 파일에 x가 있어야 실행 권한이 있는 것이다. 리눅스 파일 권한(`chmod`)에 대해 더 궁금하시다면 관련 문서를 참고해 보세요.

만약에 없다면, 다음과 같이 실행 권한을 추가하면 된다.
